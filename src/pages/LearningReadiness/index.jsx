      ...
      <div className={styles.scrollBody}>
        ...
        <div className={styles.banner}>
          <div className={styles.bannerTitle}>
            <Lightbulb size={14} className={styles.bannerIcon} />
            <span>
              本对话将重点练习以下{' '}
              <strong className={styles.bannerCount}>{total}</strong>{' '}
              个核心词汇
            </span>
          </div>
          <div className={styles.wordTags}>
            {currentWords.map((w, i) => (
              <span key={i} className={styles.wordTag}>{w.word}</span>
            ))}
          </div>
          {revealedCount > 0 && (
            <div className={styles.progressArea}>
              <div className={styles.progressLabels}>
                <span>已预览 {revealedCount}/{total}</span>
                <span>{Math.round((revealedCount / total) * 100)}%</span>
              </div>
        ...
        <div className={styles.hintCard}>
          <p>💡 若选「提前学习」，则进入词汇闪卡练习；若选「直接开始」，则进入对话情景</p>
        </div>
      </div>
